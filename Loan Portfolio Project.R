...
#I really wanted to look at the ratio of loan approvals by Gender/Marriage,
#and run a hypothesis test to see if they were significantly different
Status1 = subset(loans_train$Status, loans_train$Gender != "")
Status2 = subset(loans_train$Status, loans_train$Married != "")
Gender = subset(loans_train$Gender, loans_train$Gender != "")
Married = subset(loans_train$Married, loans_train$Married != "")


loan_gender_table = table(Status1, Gender)
prop.table(loan_gender_table, margin = 2)

loan_married_table = table(Status2, Married)
prop.table(loan_married_table, margin = 2)
...
prop.test(x = c(loan_married_table[1, 1], loan_married_table[1, 2]), n = 
            c((loan_married_table[1, 1] + loan_married_table[2, 1]), (loan_married_table[1, 2] + loan_married_table[2, 2])),
          alternative = 'two.sided', conf.level = .95)
#At 95% confidence, we can say that marriage does impact the possibility of receiving a loan (possibly due to the addition of a 
#spouses income?)
